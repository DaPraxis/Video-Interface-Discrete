...
if (process.env.NODE_ENV === 'development') {
  ...
  lightning_hole_img = "../.." + "/assets/lightning_hole.png";
...
export default function WhereInstructions(props) {
  const classes = useStyles();
  const { t } = useTranslation();
  let [redirectAddress, setRedirectAddress] = useState("");
  let [pageNum, setPageNum] = useState(0);
  let instructionsArray = t("games.TagMeWhere.ins").split("\n\n");
      ...
      <div className={classes.padding}>
        <div style={{ width: "100%", margin: "auto" }}>
          ...
          >
            <img
              src={check_mole_img}
              className={`${classes.img} ${classes.leftImg}`}
              alt="checkmole"
            />
            <img
              src={lightning_hole_img}
              className={`${classes.img} ${classes.rightImg}`}
              alt="mole_hole"
            />
          </Grid>
        </div>
  ...
  //     instructionsText={t("games.TagMeWhere.ins")}
  ...
  //     <img
  //       src={lightning_hole_img}
  //       style={{ position: "absolute", height: "75%", top: "50%" , right: "90%"}}
  //       alt="mole_hole"
  //     />
  //   </SinglePageInstructions>
  // );
